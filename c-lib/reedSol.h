...
void reedSol_Compact(struct ReedSol_Array *arr);
...
void reedSol_polDivision(
	...
	//
	reedSol_Compact(outH);
	reedSol_Compact(outF);
	*hole = outH;
	*frac = outF;
}
...
void reedSol_Euclidean(
	...
	while(a1->length > errCount){
		...
		for(int32_t x = 0;x < b0->length;x++){
			for(int32_t y = 0;y < full->length;y++){
				calc = reedsol_Mul(full->data[y],b0->data[x]);
				b1->data[x + y] = reedsol_Add(calc,b1->data[x + y]);
			}
		}
		b1->length = b0->length + full->length - 1;
...
struct ReedSol_Array *reedSol_SplitEncode(
	...
	struct ReedSol_Array *generator = reedSol_genGenerator(redCnt);
	struct ReedSol_Array *redund = reedSol_polMod(msg,generator);
	//char *ch0 = reedSol_ToString(redund);
	//printf("red:%s\n",ch0);
	//free(ch0);
	free(generator);
	return redund;
...
Reed_Sol_Decode_Tamper:
	...
	//printf("val:%s\n",reedSol_ToString(errVal));
	//printf("loc:%s\n",reedSol_ToString(errLoc));
	...
	//printf("pos:%s\n",reedSol_ToString(errPos));
	//printf("dif:%s\n",reedSol_ToString(errDif));
	//char *ch0 = reedSol_ToString(errPos);
	//printf("loc:%s\n",ch0);
	//free(ch0);
...
struct ReedSol_Array *reedSol_SplitDecode(
		struct ReedSol_Array *msg,
		struct ReedSol_Array *red,int32_t redCnt){
	//char *ch0 = reedSol_ToString(red);
	//printf("red:%s\n",ch0);
	//free(ch0);
	struct ReedSol_Array *cp = reedSol_Copy(red);
	cp->length += msg->length;
	for(int32_t idx = 0;idx < msg->length;idx++)
		cp->data[idx + red->length] = msg->data[idx];
	struct ReedSol_Array *arr = reedSol_Decode(cp,redCnt);
	for(int32_t idx = 0;idx < msg->length;idx++){
		arr->data[idx] = arr->data[idx + redCnt];
	}
...
int main(int argc,char **argv){
	...
	for(int32_t idx = 0;idx < enc->length;idx++)
		enc->data[idx] = dat[idx];
	dec = reedSol_Decode(enc,red);
	ch1 = reedSol_ToString(enc);
	ch2 = reedSol_ToString(dec);
	printf("%s\n",ch1);
	printf("%s\n",ch2);
	free(ch1);
	free(ch2);// */
	...
	for(int32_t cnt = 0;cnt < 1000;cnt++){
		int32_t pos = rand() % tst->length;
		tst->data[pos] = rand() & 0xff;
		//
		enc = reedSol_Encode(tst,red);
		for(int32_t mut = 0;mut < red / 2;mut++){
			pos = rand() % tst->length;
			enc->data[pos] = rand() & 0xff;
		}
		dec = reedSol_Decode(enc,red);
		//
		for(int32_t idx = 0;idx < tst->length;idx++){
			dec->data[idx] = dec->data[idx + red];
		}
		dec->length = tst->length;
		//
		for(int32_t idx = 0;idx < tst->length;idx++){
			if(tst->data[idx] == dec->data[idx])continue;
			ch0 = reedSol_ToString(tst);
			ch1 = reedSol_ToString(enc);
			ch2 = reedSol_ToString(dec);
			printf("%s\n",ch0);
			printf("%s\n",ch1);
